//! epic 7114 (sc-7120): Z-Image-Turbo unified curated-sampler smoke over the public `generate()`
//! path (real weights). Proves the P3 flow-match adoption for the **OneMinusSigma** convention DiT:
//!
//! - **N1 (default no-op):** an unset `req.sampler` resolves to the curated Euler integrator over the
//!   static-shift (`shift=3.0`) schedule — the same schedule the legacy `FlowMatchEuler` loop drove —
//!   so it is byte-identical to an explicit `sampler: "euler"`.
