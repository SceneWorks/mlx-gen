...
/// Per-component on-disk footprint (sc-10894) for the MLX fit-gate's staged-residency split — the Qwen
/// text encoder (`text_encoder/`), the DiT (`transformer/`), and the VAE (`vae/`), summed from the exact
/// snapshot subdirs [`crate::loader`] loads. Shared by every Z-Image engine id (base/turbo, ±control);
/// the control variants' separate control checkpoint is folded in by the worker, not here.
