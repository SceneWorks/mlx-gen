...
fn registered_descriptors_conform() {
    // The `as _` link above must have registered at least one captioner — guards the sc-4482
    // dead-strip trap (an empty registry would pass the sweep vacuously).
    assert!(
        mlx_gen::registry::captioners().next().is_some(),
        "no captioner registered — the provider registration was dead-stripped"
    );
