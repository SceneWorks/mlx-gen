//! GQA self-attention for the Ideogram 4 Qwen3-VL text encoder: GQA (32 query / 8 kv heads),
//! bias-less q/k/v/o, per-head q/k RMSNorm (on the head dim, before RoPE), HF half-split RoPE,
//! masked SDPA. Text-only path → plain 1-D RoPE. Mirrors `mlx-gen-flux2`'s `Qwen3Attention`,
//! dense-only.
