//! epic 7114 (sc-7121): Kolors unified curated-sampler + scheduler smoke over the public `generate()`
//! path (real weights). Proves the additive DDPM-cohort adoption — the curated k-diffusion path
//! (`Kolors::denoise_curated_latents` → `denoise_curated` over `DiscreteModelSampling`, ε-prediction,
//! `scaled_linear` betas over 1100 train timesteps) runs end-to-end alongside the bespoke leading-Euler
//! default, which is left byte-exact:
//!
//! - **N1 (default untouched):** an unset `req.sampler`/`req.scheduler` renders the native
//!   `euler_discrete` default — a coherent image (the legacy loop, not the curated path).
//! - **N2 (named curated sampler):** `sampler: "dpmpp_2m"` renders a coherent image that differs from
//!   the default — a real solver swap onto the unified path.
//! - **Scheduler axis:** `scheduler: "karras"` (with `euler`) renders a coherent image — the curated σ
//!   schedule flows through `schedule_sigmas`/`run_curated_sampler` end-to-end.
//!
...
/// N1: the native leading-Euler default renders a coherent image (the legacy loop, unchanged).
#[test]
#[ignore = "needs the real Kwai-Kolors/Kolors-diffusers snapshot (set KOLORS_SNAPSHOT)"]
fn default_euler_discrete_is_coherent() {
...
fn curated_dpmpp_2m_is_coherent_and_distinct() {
    ...
    assert!(
        frac > 0.01,
        "dpmpp_2m must differ from the leading-Euler default (a real curated solver); differ {frac}"
    );
}

/// Scheduler axis: the curated `karras` σ schedule (with `euler`) renders a coherent image.
#[test]
#[ignore = "needs the real Kwai-Kolors/Kolors-diffusers snapshot (set KOLORS_SNAPSHOT)"]
