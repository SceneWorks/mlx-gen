//! I2V-A14B **real-weight** end-to-end parity gate (sc-2681; `#[ignore]` — needs the 54 GB converted
//! I2V-A14B checkpoint + an input image, so it never runs in CI; the tiny `i2v_parity` gate carries
//! CI).
//!
//! The honest "real Mac e2e" for the Wan2.2-I2V-A14B port: it loads the **actual converted**
//! checkpoint and runs the genuine chain `Wan14b::generate` runs for I2V —
//!   real UMT5-XXL encode → real `build_i2v_y` (PIL-exact preprocess + z16 VAE encode + mask) →
//!   per-expert `embed_text` → boundary-switched dual-expert `denoise_moe` *with `y`* over the two
//!   real 40-layer / in_dim-36 experts → real z16 VAE decode
//! — comparing the preprocessed image, the conditioning `y`, the final latents, and the decoded
//! frames against a golden dumped from the `mlx_video` Python reference on the same converted weights,
//! the same input image, and the same injected noise (`tools/dump_i2v_real_fixtures.py`).
//!
//! Run it (after `dump_i2v_real_fixtures.py` wrote the fixture):
//! ```text
//! WAN_I2V_MODEL_DIR=~/.cache/mlx-gen-models/wan2_2_i2v_a14b_mlx_bf16 \
//! WAN_I2V_FIXTURE=/tmp/wan_i2v.safetensors \
//!   cargo test -p mlx-gen-wan --test i2v_real_parity -- --ignored --nocapture
//! ```

use std::path::PathBuf;

use mlx_gen::weights::Weights;
use mlx_gen::Image;
use mlx_gen_wan::config::WanModelConfig;
use mlx_gen_wan::pipeline::{build_i2v_y, denoise_moe, preprocess_i2v_image};
use mlx_gen_wan::scheduler::SolverKind;
use mlx_gen_wan::{decode_to_frames, load_tokenizer, Expert, Umt5Encoder, WanTransformer, WanVae};

// Must match tools/dump_i2v_real_fixtures.py.
const PROMPT: &str = "a red fox trotting across a snowy meadow at sunrise, cinematic";
const FRAMES: usize = 5;
const HEIGHT: u32 = 128;
const WIDTH: u32 = 128;
const STEPS: usize = 6;

fn env_path(var: &str) -> Option<PathBuf> {
    std::env::var_os(var).map(|s| PathBuf::from(shellexpand_home(&s.to_string_lossy())))
}

fn shellexpand_home(s: &str) -> String {
    if let Some(rest) = s.strip_prefix("~/") {
        if let Some(home) = std::env::var_os("HOME") {
            return format!("{}/{rest}", home.to_string_lossy());
        }
    }
    s.to_string()
}

fn diff(got: &[f32], exp: &[f32]) -> (f32, f64) {
    let mut max_abs = 0f32;
    let mut sum_abs = 0f64;
    let mut sum_ref = 0f64;
    for (g, e) in got.iter().zip(exp.iter()) {
        let d = (g - e).abs();
        max_abs = max_abs.max(d);
        sum_abs += d as f64;
        sum_ref += e.abs() as f64;
    }
    (max_abs, sum_abs / sum_ref.max(1e-9))
}

fn prepend1(shape: &[i32]) -> Vec<i32> {
    let mut s = vec![1];
    s.extend_from_slice(shape);
    s
}

#[test]
#[ignore = "needs the 54 GB converted Wan2.2-I2V-A14B checkpoint (WAN_I2V_MODEL_DIR + WAN_I2V_FIXTURE)"]
fn wan_i2v_a14b_real_weight_e2e_matches_reference() {
    let model_dir = match env_path("WAN_I2V_MODEL_DIR") {
        Some(p) => p,
        None => {
            eprintln!("skip: set WAN_I2V_MODEL_DIR to the converted I2V-A14B model dir");
            return;
        }
    };
    let fixture = match env_path("WAN_I2V_FIXTURE") {
        Some(p) => p,
        None => {
            eprintln!("skip: set WAN_I2V_FIXTURE (run tools/dump_i2v_real_fixtures.py first)");
            return;
        }
    };

    let cfg = WanModelConfig::from_model_dir(&model_dir).expect("read config.json");
    assert!(cfg.dual_model, "expected the dual-expert I2V-A14B config");
    assert!(
        cfg.is_i2v_concat(),
        "expected the channel-concat I2V config"
    );
    let (low_gs, high_gs) = match cfg.sample_guide_scale {
        mlx_gen_wan::GuideScale::Dual { low, high } => (low, high),
        mlx_gen_wan::GuideScale::Single(s) => (s, s),
    };

    let fx = Weights::from_file(&fixture).expect("read fixture (run dump_i2v_real_fixtures.py)");
    let img_bytes = fx.require("img_uint8").unwrap();
    let exp_chw = fx.require("img_chw").unwrap();
    let exp_y = fx.require("y").unwrap();
    let noise = fx.require("noise").unwrap();
    let exp_ctx = fx.require("context").unwrap();
    let exp_ctx_null = fx.require("context_null").unwrap();
    let exp_lat = fx.require("final_latents").unwrap();
    let exp_vid = fx.require("video").unwrap();

    // Raw input image: img_uint8 is [ih, iw, 3].
    let ish = img_bytes.shape();
    let image = Image {
        width: ish[1] as u32,
        height: ish[0] as u32,
        pixels: img_bytes.as_slice::<u8>().to_vec(),
    };

    // --- (1) Real-scale preprocess parity (PIL-exact cover-fit + crop) ---
    let img_chw = preprocess_i2v_image(&image, WIDTH, HEIGHT).expect("preprocess");
    assert_eq!(img_chw.shape(), exp_chw.shape(), "img_chw shape");
    let (px_max, _) = diff(img_chw.as_slice::<f32>(), exp_chw.as_slice::<f32>());
    println!("[preprocess] max|Δ|={px_max:.3e}");

    // --- (2) Real-weight UMT5 encode parity ---
    let tokenizer = load_tokenizer(model_dir.join("tokenizer.json"), cfg.text_len).unwrap();
    let t5_w = Weights::from_file(model_dir.join("t5_encoder.safetensors")).expect("t5 weights");
    let enc = Umt5Encoder::from_weights(&t5_w, &cfg).expect("umt5");
    let context = enc.encode(&tokenizer, PROMPT).unwrap();
    let context_null = enc.encode(&tokenizer, &cfg.sample_neg_prompt).unwrap();
    let (cx_max, cx_mr) = diff(context.as_slice::<f32>(), exp_ctx.as_slice::<f32>());
    let (cn_max, cn_mr) = diff(
        context_null.as_slice::<f32>(),
        exp_ctx_null.as_slice::<f32>(),
    );
    println!("[t5 context]      max|Δ|={cx_max:.3e} mean_rel={cx_mr:.3e}");
    println!("[t5 context_null] max|Δ|={cn_max:.3e} mean_rel={cn_mr:.3e}");
    drop(enc);
    drop(t5_w);

    // --- (3) build_i2v_y parity (real z16 VAE encode + mask + concat) ---
    let vae_w = Weights::from_file(model_dir.join("vae.safetensors")).expect("vae");
    let vae = WanVae::from_weights(&vae_w).expect("vae (with encoder)");
    let y = build_i2v_y(&vae, &image, FRAMES, HEIGHT, WIDTH, cfg.vae_stride).expect("build_i2v_y");
    assert_eq!(y.shape(), exp_y.shape(), "y shape");
    let (y_max, y_mr) = diff(y.as_slice::<f32>(), exp_y.as_slice::<f32>());
    println!(
        "[build_i2v_y] shape={:?} max|Δ|={y_max:.3e} mean_rel={y_mr:.3e}",
        y.shape()
    );
    drop(vae_w);

    // --- (4) Both real experts + boundary-switched dual MoE denoise WITH y on the injected noise ---
    let low_w = Weights::from_file(model_dir.join("low_noise_model.safetensors")).expect("low");
    let high_w = Weights::from_file(model_dir.join("high_noise_model.safetensors")).expect("high");
    let low_dit = WanTransformer::from_weights(&low_w, &cfg).expect("low DiT");
    let high_dit = WanTransformer::from_weights(&high_w, &cfg).expect("high DiT");

    let low = Expert {
        transformer: &low_dit,
        ctx_cond: low_dit.embed_text(&context).unwrap(),
        ctx_uncond: Some(low_dit.embed_text(&context_null).unwrap()),
        guidance: low_gs,
    };
    let high = Expert {
        transformer: &high_dit,
        ctx_cond: high_dit.embed_text(&context).unwrap(),
        ctx_uncond: Some(high_dit.embed_text(&context_null).unwrap()),
        guidance: high_gs,
    };
    let boundary_timestep = cfg.boundary * cfg.num_train_timesteps as f32;

    let latents = denoise_moe(
        &low,
        &high,
        boundary_timestep,
        SolverKind::UniPC,
        cfg.num_train_timesteps,
        STEPS,
        cfg.sample_shift,
        noise,
        Some(&y),
        &mut |i| println!("  step {i}/{STEPS}"),
    )
    .expect("denoise_moe");
    assert_eq!(latents.shape(), exp_lat.shape(), "final latent shape");
    let (la_max, la_mr) = diff(latents.as_slice::<f32>(), exp_lat.as_slice::<f32>());
    println!(
        "[i2v latents] shape={:?} max|Δ|={la_max:.3e} mean_rel={la_mr:.3e}",
        latents.shape()
    );
    drop(low_dit);
    drop(high_dit);
    drop(low_w);
    drop(high_w);

    // --- (5) Real z16 VAE decode ---
    let vae_w = Weights::from_file(model_dir.join("vae.safetensors")).expect("vae");
    let vae = WanVae::from_weights(&vae_w).expect("vae");
    let video = vae
        .decode(&latents.reshape(&prepend1(latents.shape())).unwrap())
        .unwrap();
    assert_eq!(video.shape(), exp_vid.shape(), "video shape");
    let (vid_max, vid_mr) = diff(video.as_slice::<f32>(), exp_vid.as_slice::<f32>());
    println!(
        "[i2v video]   shape={:?} max|Δ|={vid_max:.3e} mean_rel={vid_mr:.3e}",
        video.shape()
    );

    // Exercise the product frame-assembly path too.
    let frames_u8 = decode_to_frames(&vae, &latents, None).unwrap();
    let images = mlx_gen_wan::frames_to_images(&frames_u8).unwrap();
    assert_eq!(images.len(), exp_vid.shape()[2] as usize, "frame count");
    println!(
        "[frames] {} frames @ {}x{}",
        images.len(),
        images[0].width,
        images[0].height
    );

    // Preprocess is PIL-exact; T5 is bf16-GEMM cross-build drift; the I2V `y` rides the f32 VAE; the
    // dual-expert 40-layer stack compounds the 0.31.1-vs-0.31.2 NAX kernel diff. A wiring / conversion
    // / y-concat / channel-order bug gives mean_rel ~O(1), not a few e-2.
    assert!(
        px_max < 1e-4,
        "preprocess not PIL-exact: max|Δ|={px_max:.3e}"
    );
    assert!(cx_mr < 1e-2, "t5 context diverged: mean_rel={cx_mr:.3e}");
    assert!(
        cn_mr < 1e-2,
        "t5 context_null diverged: mean_rel={cn_mr:.3e}"
    );
    assert!(y_mr < 1e-2, "i2v y diverged: mean_rel={y_mr:.3e}");
    assert!(la_mr < 8e-2, "i2v latents diverged: mean_rel={la_mr:.3e}");
    assert!(vid_mr < 8e-2, "i2v video diverged: mean_rel={vid_mr:.3e}");
}
