//! SD3.5-Medium end-to-end real-weight smoke (M3, sc-7869) — the END-TO-END BACKSTOP for M2's
//! MMDiT-X dual-attention forward. M2's forward could not be numerically A/B'd vs diffusers (no torch
//! env), so a COHERENT real-weight Medium image out of this pipeline is the proof the MMDiT-X forward,
//! the (first-13-block) dual-attention, the triple-TE, flow-match-Euler, CFG, and the VAE de-norm are
//! all correct (exactly how E5 backstopped E3 for Large).
//!
//! `#[ignore]`d — needs the real `stabilityai/stable-diffusion-3.5-medium` snapshot in the HF cache
...
/// Resolve the SD3.5-Medium snapshot dir: `SD3_MEDIUM_SNAPSHOT` override, else the first snapshot in
/// the HF hub cache.
fn snapshot() -> PathBuf {
    if let Ok(p) = std::env::var("SD3_MEDIUM_SNAPSHOT") {
        return PathBuf::from(p);
    }
    let home = std::env::var("HOME").expect("HOME");
    let snaps = PathBuf::from(home)
        .join(".cache/huggingface/hub/models--stabilityai--stable-diffusion-3.5-medium/snapshots");
    std::fs::read_dir(&snaps)
        .unwrap_or_else(|_| panic!("no SD3.5-Medium snapshots under {snaps:?}"))
        .filter_map(|e| e.ok())
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("set SD3_MEDIUM_SNAPSHOT or populate the HF hub cache")
}
...
/// signal. White noise has a HIGH gradient (~85/255); a flat image ≈0; a coherent photo is
/// LOW-but-nonzero (smooth regions + edges), typically ~5–25.
...
/// Drive the public load→generate path for `sd3_5_medium` at a given resolution / steps / quant and
/// assert the output is a coherent image (not constant, not pure noise). Saves the PNG and prints the
/// sanity signals.
fn run_smoke(label: &str, w: u32, h: u32, steps: u32, quant: Quant, guidance: f32) {
    let snap = snapshot();
    eprintln!(
        "\n=== SD3.5-Medium e2e smoke [{label}] {w}x{h} steps={steps} quant={quant:?} \
         guidance={guidance} ===\nsnapshot: {snap:?}"
    );
    ...
    let out_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join(format!("../tools/golden/sd3_5_medium_{label}.png"));
    ...
    assert!(
        ...
        "[{label}] neighbor gradient {grad:.2} near the white-noise floor (~85) — the render is \
         noise, NOT a coherent image. This means a bug in the MMDiT-X forward / dual-attention / \
         sampler / CFG / VAE de-norm."
    );
    assert!(
        grad > 1.0,
...
/// PRIMARY ACCEPTANCE: a real-weight 1024²/40-step true-CFG Medium render. Q8 keeps the 2.5B model +
/// 3 TEs comfortably in budget. This is the AC gate + the M2 forward backstop.
#[test]
#[ignore = "needs the SD3.5-Medium snapshot (set SD3_MEDIUM_SNAPSHOT) + Metal"]
fn e2e_medium_1024_q8() {
    run_smoke("1024_q8", 1024, 1024, 40, Quant::Q8, 5.0);
}

/// Faster correctness pre-check at a smaller resolution / fewer steps (Q4) — proves the path before
/// the full 1024²/40 run. Kept as a separate `#[ignore]` test so either can be run alone.
#[test]
#[ignore = "needs the SD3.5-Medium snapshot (set SD3_MEDIUM_SNAPSHOT) + Metal"]
fn e2e_medium_512_q4_quick() {
    run_smoke("512_q4", 512, 512, 28, Quant::Q4, 5.0);
}

/// HIGHER-RES acceptance: 1440² (the validated Medium ceiling — pos_embed_max_size 384 spans a 90×90
/// patch grid). Guarded against the Mac activation budget (2K is activation-bound — the SCAIL-2 /
/// SeedVR2 lesson — so the descriptor caps at 1440). Q8 to keep activations in budget.
#[test]
#[ignore = "needs the SD3.5-Medium snapshot (set SD3_MEDIUM_SNAPSHOT) + Metal"]
fn e2e_medium_1440_q8_highres() {
    run_smoke("1440_q8", 1440, 1440, 40, Quant::Q8, 5.0);
}
