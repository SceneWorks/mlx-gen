//! SD3.5-Large-**Turbo** end-to-end real-weight smoke (E6, sc-7865) — the correctness gate for the
//! distilled few-step / CFG-off path. Turbo shares the Large MMDiT backbone (same converter, loader,
//! transformer forward, VAE), so a COHERENT image out of the 4-step / guidance-1.0 (CFG-off) recipe
//! is the proof the distilled-sampling path is wired correctly: ONE forward per step (no uncond), 4
//! steps, the same static shift 3.0 schedule.
//!
//! `#[ignore]`d — needs the real `stabilityai/stable-diffusion-3.5-large-turbo` snapshot in the HF
//! cache (or `SD3_LARGE_TURBO_SNAPSHOT`) and Metal. Run with:
//!   cargo test -p mlx-gen-sd3 --release --test e2e_turbo_real_weights -- --ignored --nocapture
//!
//! The smoke drives the PUBLIC registry path (`mlx_gen::load("sd3_5_large_turbo", spec).generate`),
...
/// Drive the public load→generate path for Turbo at a given resolution / steps / quant with CFG OFF
/// (no guidance, no negative prompt — the distilled recipe) and assert the output is a coherent image
/// (not constant, not pure noise). Saves the PNG and prints the sanity signals.
fn run_turbo_smoke(label: &str, w: u32, h: u32, steps: u32, quant: Quant) {
    let snap = snapshot();
    eprintln!(
        "\n=== SD3.5-Large-Turbo e2e smoke [{label}] {w}x{h} steps={steps} quant={quant:?} \
         CFG=off (guidance 1.0, no uncond forward) ===\nsnapshot: {snap:?}"
    );

    // Reference a crate symbol so the generator's `inventory::submit!` static is linked (see the
    // `use mlx_gen_sd3 as sd3` note above).
    assert_eq!(sd3::TURBO_MODEL_ID, "sd3_5_large_turbo");

    let spec = LoadSpec::new(WeightsSource::Dir(snap)).with_quant(quant);
    let t_load = std::time::Instant::now();
    let generator = mlx_gen::load(sd3::TURBO_MODEL_ID, &spec).expect("load sd3_5_large_turbo");
    eprintln!("loaded in {:.1}s", t_load.elapsed().as_secs_f32());

    // CFG-off: NO guidance override (defaults to 1.0) and NO negative prompt (the distilled Turbo
    // does not support either; supplying them would be a validation error).
    ...
    let out_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join(format!("../tools/golden/sd3_5_large_turbo_{label}.png"));
    ...
    assert!(
        ...
        "[{label}] neighbor gradient {grad:.2} near the white-noise floor (~85) — the render is \
         noise, NOT a coherent image. This means a bug in the distilled Turbo recipe (CFG-off path / \
         step count) or the shared MMDiT forward / sampler / VAE de-norm."
    );
    assert!(
        grad > 1.0,
...
/// PRIMARY ACCEPTANCE: a real-weight 1024² / 4-step / CFG-off Turbo render. Q8 to fit the 8.1B model
/// + 3 TEs comfortably. This is the E6 AC gate.
#[test]
#[ignore = "needs the SD3.5-Large-Turbo snapshot (set SD3_LARGE_TURBO_SNAPSHOT) + Metal"]
fn e2e_turbo_1024_q8() {
    run_turbo_smoke("1024_q8", 1024, 1024, 4, Quant::Q8);
}

/// Faster correctness pre-check at a smaller resolution (Q4) — proves the path before the full
/// 1024²/4 run. Kept as a separate `#[ignore]` test so either can be run alone.
#[test]
#[ignore = "needs the SD3.5-Large-Turbo snapshot (set SD3_LARGE_TURBO_SNAPSHOT) + Metal"]
fn e2e_turbo_512_q4_quick() {
    run_turbo_smoke("512_q4", 512, 512, 4, Quant::Q4);
}
