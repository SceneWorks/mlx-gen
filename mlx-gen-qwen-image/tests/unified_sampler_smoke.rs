//! epic 7114 (sc-7120): Qwen-Image unified curated-sampler smoke over the public `generate()` path
//! (real weights). Proves the P3 flow-match adoption end-to-end:
//!
//! - **N1 (default no-op):** an unset `req.sampler` resolves to the curated Euler integrator over the
//!   production `qwen_scheduler` — the same schedule the legacy `FlowMatchSampler` drove — so it is
//!   byte-identical to an explicit `sampler: "euler"` (the framework-level Euler-vs-legacy equivalence
//!   is gated separately in `mlx_gen::sampler` tests; this confirms the *engine* default routes there).
...
/// image has a broad histogram (std well above flat) AND spatial smoothness (small adjacent Δ); pure
/// noise has a *high* adjacent Δ (~85), a flat fill has std≈0 (mirrors `lightning_render`'s gate).
...
#[test]
#[ignore = "needs the real Qwen/Qwen-Image snapshot"]
