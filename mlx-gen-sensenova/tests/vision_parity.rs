...
fn vision_embedder_matches_reference() {
    ...
    let emb =
        NeoVisionEmbedder::from_weights(&w, &cfg, "vision_model.embeddings").expect("build embedder");

    let pixel_values = w.require("input.pixel_values").unwrap().clone();
    // The fixture is a single 4×4 patch grid.
    let grid_raw = w.require("input.grid_hw").unwrap().as_slice::<i32>().to_vec();
    ...
    println!("vis.embeds: peak|Δ|={abs:.3e}  peak-rel={rel:.3e}  shape={:?}", got.shape());
