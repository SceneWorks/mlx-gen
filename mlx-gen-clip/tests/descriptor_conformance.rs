...
fn registered_descriptors_conform() {
    // The `as _` link above must have registered at least one image embedder — guards the sc-4482
    // dead-strip trap (an empty registry would pass the sweep vacuously).
    assert!(
        mlx_gen::registry::image_embedders().next().is_some(),
        "no image embedder registered — the provider registration was dead-stripped"
    );
