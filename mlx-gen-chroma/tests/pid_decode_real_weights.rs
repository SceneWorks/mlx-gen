//! sc-7846 e2e: the **integrated** PiD decode path for Chroma — load the real `Chroma1-Flash` snapshot
//! with a PiD decoder overlay (`LoadSpec::with_pid`) and run `Generator::generate` once for the VAE
//! baseline and once with `use_pid`, proving the live denoised latent routes through `Chroma::decode`'s
//! shared `LatentDecoder` seam into a 4× super-resolved PiD image. Chroma is a FLUX.1-schnell derivative
//! whose VAE is loaded by `mlx_gen_flux::load_vae` (byte-identical FLUX.1 16-ch `AutoencoderKL`), so it
//! reuses the `flux` PiD student — and, like Z-Image, its latents come from a different upstream
//! transformer than FLUX.1, so this is a per-model leg of the `flux`-backbone gate. Uses the few-step
//! Flash variant to bound denoise cost.
//!
//! `#[ignore]`d — needs a `Chroma1-Flash` snapshot (env `CHROMA_FLASH_DIR`, else the
//! `lodestones/Chroma1-Flash` HF cache), the converted flux PiD checkpoint (env `PID_FLUX_SAFETENSORS`,
//! else `tools/golden/pid/flux_2k.safetensors`), and a `gemma-2-2b-it` snapshot dir (env
...
use mlx_gen_chroma::load_flash;

fn env_path(name: &str) -> Option<PathBuf> {
    std::env::var(name).ok().map(PathBuf::from)
}

fn chroma_flash_dir() -> PathBuf {
    if let Some(p) = env_path("CHROMA_FLASH_DIR") {
        return p;
    }
    let home = std::env::var("HOME").unwrap();
    let snaps = PathBuf::from(home)
        .join(".cache/huggingface/hub/models--lodestones--Chroma1-Flash/snapshots");
    std::fs::read_dir(&snaps)
        .expect("Chroma1-Flash HF cache snapshots dir (or set CHROMA_FLASH_DIR)")
        .filter_map(|e| e.ok())
        .map(|e| e.path())
        .find(|p| p.is_dir())
        .expect("a Chroma1-Flash snapshot dir")
}
...
#[test]
#[ignore = "needs a Chroma1-Flash snapshot + converted flux PiD checkpoint + gemma-2-2b-it"]
fn chroma_flash_pid_decode_vs_vae() {
    let size: u32 = std::env::var("CHROMA_PID_SIZE")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(512);

    let spec = LoadSpec::new(WeightsSource::Dir(chroma_flash_dir())).with_pid(
        WeightsSource::File(pid_checkpoint()),
        WeightsSource::Dir(gemma_dir()),
    );

    eprintln!("loading Chroma1-Flash (+PiD overlay), size={size} ...");
    let t = Instant::now();
    let model = load_flash(&spec).expect("load Chroma1-Flash + PiD");
    ...
    save_png(&vae_img, &format!("{dir}/chroma_vae_{}.png", vae_img.width));
    save_png(&pid_img, &format!("{dir}/chroma_pid_{}.png", pid_img.width));
    eprintln!(
        "wrote {dir}/chroma_vae_{}.png + chroma_pid_{}.png  (PiD {:.1}× slower than VAE)",
        vae_img.width,
        pid_img.width,
